...
import { Lock, Unlock, Play, Pause, Volume2, VolumeX, Maximize, MessageSquare, Flame, Heart, Flower2, Share2, Send, Clock, MapPin, Sparkles, AlertCircle, CheckCircle2, User, ChevronRight, X } from 'lucide-react';
    ...
    <div className={`relative min-h-[600px] w-full rounded-3xl overflow-hidden border ${
      ...
      {!isAuthenticated ? (
            ...
            <form onSubmit={handleVerifyPin} className="space-y-3">
              <div>
                <input
                  ...
                  placeholder="Ingrese el PIN (Ej: 8492)"
                  className={`w-full text-center text-xl tracking-[0.3em] font-mono py-3 rounded-xl border ${
                    pinError
                      ? 'border-red-500 bg-red-500/10 text-red-400'
                      : isDark ? 'bg-stone-950 border-stone-750 text-white' : 'bg-stone-50 border-stone-300 text-stone-900'
                  } focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all`}
                />
                {pinError && (
                  <p className="text-xs text-red-500 mt-1.5 font-medium">
                    PIN incorrecto. Ingrese el código proporcionado por la familia. (PIN Demo: {serviceData.accessPin})
                  </p>
                )}
              </div>
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-amber-700 hover:bg-amber-600 text-white font-semibold text-sm shadow-md transition-colors flex items-center justify-center gap-2"
              >
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
          ...
          <div className="lg:col-span-8 bg-black flex flex-col justify-between relative overflow-hidden min-h-[350px] sm:min-h-[460px]">
            ...
            <div className="relative z-20 p-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-center justify-between text-white text-xs mt-auto">
              <div className="flex items-center gap-3">
                <button
              ...
              <div className="flex items-center gap-2">
                <span className="flex items-center gap-1 text-[11px] bg-red-600/90 text-white font-bold px-2 py-0.5 rounded-md font-mono">
                  🔴 {viewerCount} presentes online
                </span>
              </div>
            </div>
